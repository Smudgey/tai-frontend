...
class TaxAccountSummaryViewModelSpec extends PlaySpec with FakeTaiPlayApplication with I18nSupport {
  ...
  "TaxAccountSummaryViewModel apply method" must {
    "return a view model" which {
      "has header relating to current tax year" in {
        ...
        val sut = TaxAccountSummaryViewModel(FullTaxSummaryForYearResponse(
              emptyTaxCodeIncomes, emptyEmployments, taxAccountSummary, isAnyFormInProgress = true, Incomes(Seq.empty, nonTaxCodeIncome)))
      ...
      "has correctly formatted negative tax free amount and estimated income" when {
        "taxAccountSummary has a negative values" in {
          val sut = TaxAccountSummaryViewModel(FullTaxSummaryForYearResponse(
            ...
            Incomes(Seq.empty, nonTaxCodeIncome)))
      ...
      "has correctly formatted zero tax free amount and estimated income" when {
        "taxAccountSummary has zero values" in {
          val sut = TaxAccountSummaryViewModel(FullTaxSummaryForYearResponse(
            emptyTaxCodeIncomes, emptyEmployments, TaxAccountSummary(0, 0, 0, 0, 0), isAnyFormInProgress = true, Incomes(Seq.empty, nonTaxCodeIncome)))
      ...
      "has empty employments, pensions and ceasedEmployments list" when {
        "taxCodeIncomes and employments are empty" in {
          val sut = TaxAccountSummaryViewModel(FullTaxSummaryForYearResponse(
            emptyTaxCodeIncomes, emptyEmployments, TaxAccountSummary(0, 0, 0, 0, 0), isAnyFormInProgress = true, Incomes(Seq.empty, nonTaxCodeIncome)))
          sut.employments.isEmpty mustBe true
          sut.pensions.isEmpty mustBe true
          sut.ceasedEmployments.isEmpty mustBe true
        }
        ...
        "taxCodeIncomes doesn't have the corresponding element as employments, and none of the employments have an end date" in {
          ...
          val sut = TaxAccountSummaryViewModel(FullTaxSummaryForYearResponse(
            emptyTaxCodeIncomes, employmentsWithNoEndDate, TaxAccountSummary(0, 0, 0, 0, 0), isAnyFormInProgress = true, Incomes(Seq.empty, nonTaxCodeIncome)))
          sut.employments.isEmpty mustBe true
          sut.pensions.isEmpty mustBe true
          sut.ceasedEmployments.isEmpty mustBe true
        }
        ...
        "employments doesn't have the corresponding element as taxCodeIncomes" in {
          val sut = TaxAccountSummaryViewModel(FullTaxSummaryForYearResponse(
            taxCodeIncomes, emptyEmployments, TaxAccountSummary(0, 0, 0, 0, 0), isAnyFormInProgress = true, Incomes(Seq.empty, nonTaxCodeIncome)))
          sut.employments.isEmpty mustBe true
          sut.pensions.isEmpty mustBe true
          sut.ceasedEmployments.isEmpty mustBe true
        }
      ...
      "has an employment list" when {
        "taxCodeIncomes and employments have matching employments" in {
          val sut = TaxAccountSummaryViewModel(FullTaxSummaryForYearResponse(
            taxCodeIncomes, employments, TaxAccountSummary(0, 0, 0, 0, 0), isAnyFormInProgress = true, Incomes(Seq.empty, nonTaxCodeIncome)))
      ...
      "has the iya banner boolean set to true" when {
        "tax account summary shows an in year adjustment value greater than zero" in {
          val sut = TaxAccountSummaryViewModel(FullTaxSummaryForYearResponse(
            taxCodeIncomes, employments, TaxAccountSummary(0, 0, 0.01, 0.02, 0.01), isAnyFormInProgress = true, Incomes(Seq.empty, nonTaxCodeIncome)))
      ...
      "has the iya banner boolean set to false" when {
        "tax account summary shows an in year adjsutment value of zero or less" in {
          ...
          val sutNeg = TaxAccountSummaryViewModel(FullTaxSummaryForYearResponse(
            taxCodeIncomes, employments, TaxAccountSummary(0, 0, -0.01, 0, 0), isAnyFormInProgress = true, Incomes(Seq.empty, nonTaxCodeIncome)))
      ...
      "has the other income sources with links" when {
        ...
        "other income sources with untaxed interest are available and bank accounts are also available" in {
          ...
          val sut = TaxAccountSummaryViewModel(FullTaxSummaryForYearResponse(
            emptyTaxCodeIncomes, emptyEmployments, taxAccountSummary, isAnyFormInProgress = true, Incomes(Seq.empty, nonTaxCodeIncomeWithBankAccounts)))
        ...
        "other income sources without untaxed interest are available" in {
          ...
          val sut = TaxAccountSummaryViewModel(FullTaxSummaryForYearResponse(
            emptyTaxCodeIncomes, emptyEmployments, taxAccountSummary, isAnyFormInProgress = true, Incomes(Seq.empty, nonTaxCodeIncomeWithOutUntaxedInterest)))
      ...
      "has not other income sources" when {
        "there is no other income sources are available" in {
          ...
          val sut = TaxAccountSummaryViewModel(FullTaxSummaryForYearResponse(
            emptyTaxCodeIncomes, emptyEmployments, taxAccountSummary, isAnyFormInProgress = true, Incomes(Seq.empty, nonTaxCodeIncomeWithOutAnything)))
    ...
    "return a view model instance for an end dated employment record that has no matching TaxCodeIncomeSource record" in {
      val sut = TaxAccountSummaryViewModel(FullTaxSummaryForYearResponse(
        taxCodeIncomes, nonMatchedEmployments, TaxAccountSummary(0, 0, 0, 0, 0), isAnyFormInProgress = true, Incomes(Seq.empty, nonTaxCodeIncome)))
  ...
  "IncomeSourceViewModel apply method (two parameter version)" must {
    "return a viewmodel" which {
      ...
      "has the taxCode field coming from taxCodeWithEmergencySuffix in taxCodeIncome model" in {
        val sut = IncomeSourceViewModel(taxCodeIncome.copy(basisOperation = Week1Month1BasisOperation), employment)
  ...
  val taxCodeIncomes = Seq(
    ...
    TaxCodeIncome(EmploymentIncome, None, 2222, "employment", "BR", "employer", Week1Month1BasisOperation, Live)
  )
